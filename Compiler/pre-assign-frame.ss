#|
Program name    : pre-assign-frame.ss
Functions       : pre-assign-frame
Description     : This program assign frame-vars to uvars in the spills.
Input Language  : Scheme with a frame-conflict graph consisting of conflict list for all the uvars in locals list
Output Language : Scheme with locate list
|#
...
;; Defines pass28 of the compiler
(define-who pre-assign-frame
  ...
  (define find-friend-fv
    (lambda (cvar ls conflict-entry bt)
      (cond
	[(null? ls) (if (enquire-friend bt (cdr (assq cvar bt)))
			#f 
			(car (cdar (find-homes (list cvar) conflict-list '()))))]
	[(and (frame-var? (car ls)) (not (memq (car ls) conflict-entry))) (car ls)]
	[else (find-friend-fv cvar (cdr ls) conflict-entry bt)])))
  ...
  (define update-frd-home
    (lambda (uvar reg bt)
      ...
      (if (or (assq uvar friends-home) (memq reg conflict-entry))
	  friends-home
 	  (begin
    ...
    (lambda (uvar reg bt)
      (map (lambda (entry) 
	     (let ([tail (cdr entry)])
			...
			(if (memq (car entry) spill-vars) (update-frd-home (car entry) reg bt) '())
	                entry)
	              entry))) bt)))
  ;; This function find homes for move-related variables
  (define find-mb-homes
	...
	[else (let* ([cvar (car var*)]
		     ...
		     [cf-entry (cdr (assq cvar conflict-list))])
	  		...
	  		(let ([friend-fv (find-friend-fv cvar frd-vars cf-entry bt)])
	  		     (cond
	    		       [(eq? friend-fv #f) (find-mb-homes (cdr var*) bt)]
	    		       [else 
				 (begin (set! conflict-list (update-clist (list (list cvar friend-fv)) conflict-list))
		       			(let ([updated-bt (update-bias-list cvar friend-fv bt)])
			    		     (begin
		 	      			(unless (assq cvar friends-home) 
		              			(set! friends-home (cons (list cvar friend-fv) friends-home)))
			      			(find-mb-homes (cdr var*) updated-bt))))]))))])))
  ...
  ;; This function find homes for variables other than move-related spills
  (define find-homes
    (lambda (var* ct home*)
      (if (null? var*)
          home*
        ...
        [(locals (,local* ...)
           ...
           (let* ([uvar* (union local* spill*)] 
	          ...
	          [bias-lst-wcf (filter-bias (rem-bt-conflicts bias-list conflict-list))]
 	          [biased-home* (find-mb-homes spill* bias-lst-wcf)]
	          [homes (if (null? biased-home*) '() (map car biased-home*))]
	          [home* (find-homes (difference spill* homes) conflict-list '())])
                `(locals (,local* ...)
                 (new-frames (,nfv** ...)
